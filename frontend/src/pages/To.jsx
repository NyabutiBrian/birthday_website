...
const To = () => {
    ...
    useEffect(() => {
        // Fetch articles
        fetch(`https://backend-momanyi.onrender.com/birthday/api/details/${slug}/`)
                        ...
                        <>
                            {details && (
                                <div className='flex flex-col'>
                                    {/* Name */}
                                    <h5 id='articleName' className='font-bold text-xl py-4 uppercase text-center'><span className='font-light'>{details.first_name}</span> - {details.last_name}</h5>
            
                                    {/* Image */}
                                    <div className="px-14 py-36 lg:py-40 rounded-3xl bg-center bg-cover shadow-2xl" style={{backgroundImage: `url(${details.image})`,}}></div>
